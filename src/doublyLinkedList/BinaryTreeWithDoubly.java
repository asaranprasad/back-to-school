...
public class BinaryTreeWithDoubly {
  ...
  public static DNode constructBinaryTreeWithInorderAndPreOrder(String inOrder,
      String preOrder) {
  ...
  public static String diameterOfTheBinaryTree(DNode root, int height) {
    try {
      ...
      int diameterOfLeftSubtreeOfCurrentRootNode =
          Integer.parseInt(tempString.split("-")[0]);
      lh = Integer.parseInt(tempString.split("-")[1]);

      tempString = diameterOfTheBinaryTree(root.right, rh);
      int diameterOfRightSubtreeOfCurrentRootNode =
          Integer.parseInt(tempString.split("-")[0]);
      rh = Integer.parseInt(tempString.split("-")[1]);

      height = Math.max(lh, rh) + 1;
      return Math.max(lh + rh + 1, Math.max(diameterOfLeftSubtreeOfCurrentRootNode,
          diameterOfRightSubtreeOfCurrentRootNode) + 1) + "-" + height;
    } catch (Exception e) {
  ...
  public static void main(String[] args) {
    ...
    System.out.println("\n\nDiameter of this tree: "
        + diameterOfTheBinaryTree(bt.root, 0).split("-")[0]);
    System.out.println(
        "Height of this tree: " + diameterOfTheBinaryTree(bt.root, 0).split("-")[1]);
    //
    tempString = "";
    inOrder(bt.root);
